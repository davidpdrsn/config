...
async function runCloud(io: IOBridge, context: CloudRunContext): Promise<void> {
	...
	try {
		...
		const remoteAgentInstructions = [
			...
			"If you get stuck, explain the blocker in your response and leave the tmux session available for the user.",
			...
			"Then summarize the result and confirm whether commit, bookmark move, and push succeeded in your final response.",
			"Leave the tmux session available so the user can reconnect and read the result.",
		].join("\n");
