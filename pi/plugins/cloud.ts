		...
		handler: async (_args, ctx) => {
			...
			try {
				...
				const remoteAgentInstructions = [
					...
					`4) tmux kill-session -t "$(tmux display-message -p '#S')"`,
					"Then explicitly confirm in your final response that commit, bookmark move, push, and tmux session shutdown succeeded.",
				].join("\n");
